...
Elman::Elman(unsigned int nbInputs,
		unsigned int nbOutputs,
		bool activeBiais,
		bool onlyUseBiaisForFirstHiddenLayer,
		double biaisValue) :
		MLP(nbInputs, nbOutputs, activeBiais, onlyUseBiaisForFirstHiddenLayer, biaisValue) {
	initLastOutputs();
}
...
Elman::Elman(unsigned int nbInputs,
		...
		bool onlyUseBiaisForFirstHiddenLayer,
		double biaisValue) :
		MLP(nbInputs, nbOutputs, nbNeuronsPerLayer, activeBiais, onlyUseBiaisForFirstHiddenLayer, biaisValue) {
	initLastOutputs();
}
...
void Elman::step() {
	...
	for(unsigned int k = 0; k < _nbNeuronsPerLayer.size() - 1; k++) {
		...
		unsigned int const remainingWeights = _weights.size() - weightsIndice;
